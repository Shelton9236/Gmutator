...
idx1HEADER* idx1HEADER::generate() {
	...
	for (i = 0; (i < ::g->frame_index); i++) {
			...
			GENERATE_VAR(index_entry, ::g->index_entry.generate(::g->frame()[i]->movi_chunk().id(), 0x00000000, offset, ::g->frame()[i]->movi_chunk().movi_datalen()));
			offset += (8 + ::g->frame()[i]->movi_chunk().movi_datalen());
		} else {
			GENERATE_VAR(index_entry, ::g->index_entry.generate(::g->frame()[i]->movi_list().type(), 0x00000000, offset, ::g->frame()[i]->movi_list().list_datalen()));
			offset += 12;
		};
	;
	};
...
void generate_file() {
	...
	FSeek(::g->file_end);
	while (!FEof()) {
		GENERATE(garbage, ::g->garbage.generate());
	};
